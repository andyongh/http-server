...
def make_svg_chart(title, x_label, y_label, conns, data_series, filename):
    ...
    svg = []
    svg.append(
        f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {width} {height}" width="100%" height="100%" style="background-color: #1e1e24; font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, Helvetica, Arial, sans-serif;">'
    )
    ...
    for x_val in conns:
        ...
        svg.append(
            f'<line x1="{x_pos}" y1="{top_margin}" x2="{x_pos}" y2="{height - bottom_margin}" class="grid-line" />'
        )
        svg.append(
            f'<text x="{x_pos}" y="{height - bottom_margin + 20}" text-anchor="middle" class="tick-text">{x_val}</text>'
        )

    # Axes
    svg.append(
        f'<line x1="{left_margin}" y1="{top_margin}" x2="{left_margin}" y2="{height - bottom_margin}" class="axis" />'
    )
    svg.append(
        f'<line x1="{left_margin}" y1="{height - bottom_margin}" x2="{width - right_margin}" y2="{height - bottom_margin}" class="axis" />'
    )
    ...
    for series_name, values in data_series.items():
        ...
        for pt in points:
            svg.append(
                f'<circle cx="{pt[0]:.1f}" cy="{pt[1]:.1f}" r="4" fill="{color}" stroke="#1e1e24" stroke-width="1.5" />'
            )
...
def main():
    ...
    modes_to_test = [
        ...
        {
            "name": "Lua Handler (4 Workers)",
            "workers": 4,
            "lua": "examples/handler.lua",
        },
    ]
    ...
    for mode in modes_to_test:
        ...
        try:
            ...
            _, _, verify_failed, output = run_wrk_client(
                ...
                lua_script="tests/wrk_verify.lua",
                path="/echo",
            )
    ...
    for lvl in LOG_LEVELS:
        ...
        for c in CONCURRENCY_LEVELS:
            ...
            try:
                ...
                # Run wrk client
                req_sec, bytes_sec, _, _ = run_wrk_client(
                    conns=c, duration_s=DURATION_S
                )
    ...
    for lvl in LOG_LEVELS:
        for idx, c in enumerate(CONCURRENCY_LEVELS):
            ...
            report_lines.append(
                f"| `{lvl}` | {c} | {rps:.1f} | {netio:.2f} | {cpu:.1f}% | {rss:.2f} |"
            )

    report_lines.append("\n## 3. Growth Curves")
    report_lines.append(
        "The resource usage growth curves show the performance overhead of logging. Higher logging rates dramatically increase CPU utilization and lower network throughput because of console I/O serialization."
    )
    report_lines.append("\n### CPU Utilization Curve")
    report_lines.append(
        f"![CPU Curve](file://{os.path.join(ARTIFACTS_DIR, 'cpu_curve.svg')})"
    )
    report_lines.append("\n### RAM Usage Curve")
    report_lines.append(
        f"![RAM Curve](file://{os.path.join(ARTIFACTS_DIR, 'ram_curve.svg')})"
    )
    report_lines.append("\n### Network I/O Throughput Curve")
    report_lines.append(
        f"![Network Curve](file://{os.path.join(ARTIFACTS_DIR, 'network_curve.svg')})"
    )
